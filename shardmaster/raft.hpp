...
class Raft{
public:
    static void* listenForVote(void* arg);          //二级线程
    static void* listenForAppend(void* arg);        //二级线程 
    static void* processEntriesLoop(void* arg);     //三级线程，负责心跳及调用日志发送
    static void* electionLoop(void* arg);           //三级线程，负责监控是否超时及调用拉票函数
    static void* callRequestVote(void* arg);        //四级线程，实际的call投票
    static void* sendAppendEntries(void* arg);      //四级线程，实际的call日志同步
    ...
    RequestVoteReply requestVote(RequestVoteArgs args);             //供别人远程调用的方法
    AppendEntriesReply appendEntries(AppendEntriesArgs args);       //供别人远程调用的方法
...
//只是将提交了但是还没有应用的日志加入到一个局部队列中，并逐个处理
//这里为什么不用信号来进行处理了？
void* Raft::applyLogLoop(void* arg){
    ...
    while(1){
        while(!raft->dead){
            ...
            raft->m_lock.lock();
            while(raft->m_lastApplied < raft->m_commitIndex){//还有提交了但是没有应用的
                raft->m_lastApplied++;
                int appliedIdx = raft->idxToCompressLogPos(raft->m_lastApplied);//获得在数组中的索引
                ApplyMsg msg;
                ...
                msgs.push_back(msg);//先存放到msgs队列中等待处理
            }
            raft->m_lock.unlock();
...
void* Raft::processEntriesLoop(void* arg){
    ...
    while(!raft->dead){
        ...
        for(auto& server : raft->m_peers){
            if(server.m_peerId == raft->m_peerId) continue;
            if(raft->m_nextIndex[server.m_peerId] <= raft->m_lastIncludedIndex){//发送安装日志
            /*判定何时发送的条件：这个客户端的nextIndex小于leader的快照部分了*/
                ...
                server.isInstallFlag = true;//标记该server正在安装快照
                ...
                //     raft->m_peerId, server.m_peerId, raft->m_nextIndex[server.m_peerId], raft->m_lastIncludedIndex);
                pthread_create(tid + i, NULL, sendAppendEntries, raft);//分别为每一个peers创建一个线程进行日志的发送
                pthread_detach(tid[i]);
            }
            i++;
        }
...
void* Raft::sendInstallSnapShot(void* arg){
    ...
    for(int i = 0; i < raft->m_peers.size(); i++){
        ...
        if(!raft->m_peers[i].isInstallFlag){            //如果不是要发送快照的就不要进行后面的部分了
            // printf("%d is append, continue\n", i);
            continue;
        }
        if(raft->isExistIndex.count(i)){                //如果已经发送了
    ...
    if(raft->isExistIndex.size() == raft->m_peers.size() - 1){              //这一轮已经全部结束
    ...
    args.term = raft->m_curTerm;
    raft->readSnapShot();       
    args.snapShot = raft->persister.snapShot;//填充快照
...
InstallSnapSHotReply Raft::installSnapShot(InstallSnapShotArgs args){
    ...
    if(args.lastIncludedIndex <= m_lastIncludedIndex){//如果自己的快照还要更加新，则直接返回
        m_lock.unlock();
        return reply;
    }else{
        if(args.lastIncludedIndex < lastIndex()){//如果arg的快照是自己还没快照的部分
            if(m_logs[idxToCompressLogPos(lastIndex())].m_term != args.lastIncludedTerm){//如果两者的term不同
                m_logs.clear();
            }else{
                vector<LogEntry> tmpLog(m_logs.begin() + idxToCompressLogPos(args.lastIncludedIndex) + 1, m_logs.end());//将leader已经快照的部分截断掉，只保留未快照的部分
                m_logs = tmpLog;
            }
        }else{
            m_logs.clear();
    ...
    persister.snapShot = args.snapShot;//更新自己的快照内容
    printf("in raft stall rpc, snapShot is %s\n", persister.snapShot.c_str());
    saveRaftState();
    saveSnapShot();

    m_lock.unlock();
    installSnapShotTokvServer();//不仅raft自己的要进行更新，还要对应用层进行更新
    return reply;
}
...
void* Raft::sendAppendEntries(void* arg){
    ...
    //以下只是为了找到一个比较好的clientid
    for(int i = 0; i < raft->m_peers.size(); i++){
        if(raft->m_peers[i].m_peerId == raft->m_peerId) continue;   //如果是自己本身
        if(raft->m_peers[i].isInstallFlag) continue;                //如果是在安装日志
        if(raft->isExistIndex.count(i)) continue;                   //如果已经发送过
        clientPeerId = i;
        raft->isExistIndex.insert(i);
    ...
    if(raft->isExistIndex.size() == raft->m_peers.size() - 1){      //如果所有的都已经发送完毕
        // printf("append clear size is %d\n", raft->isExistIndex.size());
        for(int i = 0; i < raft->m_peers.size(); i++){
            raft->m_peers[i].isInstallFlag = false;
        }
        raft->isExistIndex.clear();            //及时清空已发送标记
    }
    ...
    for(int i = raft->idxToCompressLogPos(args.m_prevLogIndex) + 1; i < raft->m_logs.size(); i++){
        args.m_sendLogs += (raft->m_logs[i].m_command + "," + to_string(raft->m_logs[i].m_term) + ";");//进行日志的填充
    }
    ...
    if(reply.m_success){
        ...
        int realMajorityMatchIndex = tmpIndex[tmpIndex.size() / 2];
        if(realMajorityMatchIndex > raft->m_commitIndex && (realMajorityMatchIndex <= raft->m_lastIncludedIndex || raft->m_logs[raft->idxToCompressLogPos(realMajorityMatchIndex)].m_term == raft->m_curTerm)){
            raft->m_commitIndex = realMajorityMatchIndex;//更新commitIndex
        }
    }
...
AppendEntriesReply Raft::appendEntries(AppendEntriesArgs args){
    ...
    if(args.m_term >= m_curTerm){
        if(args.m_term > m_curTerm){
            m_votedFor = -1;    //说明新的leader已经形成,自己的保存的votefor可以重置了
            saveRaftState();
        }
    ...
    if(args.m_prevLogIndex < m_lastIncludedIndex){
        ...
        m_lock.unlock();
        return reply;
    /*自己快照过的日志和pre一样，基本相符，但是两者的term是不一样的--严重问题*/
    ...
    /*自己的日志缺失很多--那么修复就行*/
        if(lastIndex() < args.m_prevLogIndex){//如果自己日志的最后一条仍然小于preLogIndex,那么说明自己的日志缺了很多
        ...
        if(m_logs[idxToCompressLogPos(args.m_prevLogIndex)].m_term != args.m_prevLogTerm){
            ...
            for(int index = m_lastIncludedIndex + 1; index <= args.m_prevLogIndex; index++){
                if(m_logs[idxToCompressLogPos(index)].m_term == reply.m_conflict_term){
                    reply.m_conflict_index = index;           //从前往后，找到冲突term的第一个index,比索引要加1
                    break;
                }
            }
            m_lock.unlock();
    ...
    if(m_commitIndex < args.m_leaderCommit){
        m_commitIndex = min(args.m_leaderCommit, lastIndex());//已经提交的日志
...
StartRet Raft::start(Operation op){
    ...
    push_backLog(log);//将待执行的日志放入到队列中
...
//为应用层安装快照：将快照加入到msgs队列中，并通过信号量唤醒
void Raft::installSnapShotTokvServer(){
    ...
    ApplyMsg msg;
    msg.commandValid = false;//标记是快照更新
