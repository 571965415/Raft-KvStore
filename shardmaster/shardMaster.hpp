...
QueryReply ShardMaster::Query(QueryArgs args){
    ...
    operation.value = "random";
    operation.clientId = args.clientId;
    operation.requestId = args.requestId;//用于去重
    operation.args = to_string(args.configNum);

    StartRet ret = m_raft.start(operation);//给raft喂进去
    ...
    m_lock.lock();
    m_requestMap.erase(ret.m_cmdIndex);//因为这个不需要去重
    m_lock.unlock();
    return reply;
}
...
void* ShardMaster::applyLoop(void* arg){
    ...
    while(1){
        ...
        if(master->m_requestMap.count(index)){//对于一般的请求需要去重，但是对弈query则不需要去重
            isOpExist = true;
        ...
        master->m_clientSeqMap[operation.clientId] = max(operation.requestId, master->m_clientSeqMap[operation.clientId]); // 这里似乎存在问题，应该用一个max
