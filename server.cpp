...
void initStr2PortMap(int num){
    ...
    for(int i = 1; i <= num; i++){
        for(int j = 1; j <= EVERY_SERVER_RAFT; j++){//每个服务器分配五个端口
...
MigrateReply str2MigrateReply(string str){
    ...
    reply.configNum = stoi(content[0]); //配置编号
    reply.shard = stoi(content[1]);     //分片号
    reply.err = content[2];             //错误号
    tmp = "";
...
private:
    ...
    unordered_map<int, unordered_map<int, unordered_map<string, string>>> toOutShards;//要发送出去的数据库内容，格式：配置编号，片号，数据库条目
    unordered_map<int, int> comeInShards;                                             //需要从其他group获取数据的片
...
/**
 * @param 当前这个组内的所有shardkv信息
 * @param 自己在这个组内的编号
 * @param gid组id从1开始
 * @param 状态机（日志）的最大长度
 * @param master那个组的相关信息
 * @brief 
*/
void ShardKv::StartKvServer(vector<kvServerInfo>& kvInfo, int me, int gid, int maxRaftState, vector<vector<int>>& masters){
    ...
    m_masterClerk.makeClerk(masters);//为master绑定组内其他server的三个独立端口

    m_raft.setRecvSem(1);
    m_raft.setSendSem(0);

    m_raft.Make(peers, me);//创建底层的raft
...
void* ShardKv::updateConfigLoop(void* arg){
    ...
    while(1){
        ...
        kv->m_lock.lock();
        if(!isleader || kv->comeInShards.size() > 0){//如果不是leader或者正准备pull拉取，就不可以query新的配置
            kv->m_lock.unlock();
            usleep(50000);
            continue;
        }
...
void* ShardKv::doPullShard(void* arg){
    ...
    int gid = config.shards[args.shard];//获取目标group的组ID
    bool isOk = false;
    for(const auto& server : config.groups[gid]){//遍历该group中的每个服务器，是不知道该组中哪个服务器是leader的，只能挨个试
        if(isOk) break;
        int port = make_end(server);
        buttonrpc client;
        client.as_client("127.0.0.1", port);//访问对应的服务器去了
        ...
        if(reply.err == OK){
            ...
            kv->m_raft.start(operation);    //raft就这个操作达成共识之后就会进行提交，提交的方式是在msgs中添加回馈数据，并且通过生产、消费者进行信号量的唤醒     
        }
    }
    kv->m_lock.lock();
...
MigrateRpcReply ShardKv::shardMigration(MigrateArgs args){
    ...
    if(!isLeader) return retReply;//如果不是leader，就直接返回了，默认为ErrWrongLeader错误

    reply.err = ErrWrongGroup;
    str = MigrateReply2Str(reply);
    retReply.reply = str;
    m_lock.lock();
    if(args.configNum >= this->m_config.configNum){//如果args的配置序号比自己目前的还要大，直接返回ErrWrongGroup错误
        m_lock.unlock();
        return retReply;
    }
    //正常
    ...
    for(const auto& data : toOutShards[args.configNum][args.shard]){
        reply.database.insert(data);//数据库填充
    }
...
void* ShardKv::garbagesCollectLoop(void* arg){
    ...
    while(1){
        ...
        for(auto a : kv->garbages){
            printf("garbages.size() : %d cfg: %d shard: ", a.second.size(), a.first);
            for(auto b : a.second){
                printf(" %d ", b);
            }
            printf("\n");
        ...
        kv->m_lock.unlock();
        for(auto garbage : kv->garbagesBackUp){//遍历每一个配置期间
            kv->m_lock.lock();
            int tmpGarbagesSize = garbage.second.size();//某一个配置期间的所有垃圾
            // kv->garbageSize = tmpGarbagesSize;
            kv->garbageIter = garbage.second.begin();
            kv->garbageConfigNum = garbage.first;//垃圾的配置期间
            kv->garbageFinished = 0;
            kv->m_lock.unlock();

            pthread_t tid[tmpGarbagesSize];
            for(int i = 0; i < tmpGarbagesSize; i++){//给每一个垃圾都进行处理
        ...
        kv->garbagesBackUp.clear();//你怎么知道别人都把垃圾处理完成了？
        usleep(100000);
    }
}
...
void* ShardKv::doGarbage(void* arg){
    ...
    kv->m_lock.lock();
    //遍历所有的垃圾
    //这个锁能保证取到的shard和configNum具有对应关系
    args.shard = (*kv->garbageIter++);
    args.configNum = kv->garbageConfigNum;
    kv->m_lock.unlock();

    Config config = kv->m_masterClerk.Query(args.configNum);//向配置存储器查询configNum的配置信息
    int gid = config.shards[args.shard];//要去和那个组沟通
    ...
    for(const auto& server : config.groups[gid]){//遍历目的组内的所有服务器，因为不知道哪个是leader
        int port = make_end(server);
        buttonrpc client;
        ...
        if(reply.err == OK){
            kv->m_lock.lock();
            kv->garbages[kv->garbageConfigNum].erase(args.shard);//删除对应的垃圾
...
GarbagesCollectReply ShardKv::garbagesCollect(GarbagesCollectArgs args){
    ...
    if(!toOutShards.count(args.configNum) or !toOutShards[args.configNum].count(args.shard)){//未查询到
        m_lock.unlock();
        return reply;
    }
...
//删除对应的分片，如果对应的配置期的垃圾都没有了，那么删除配置期
...
//配置更新导致的数据迁移：将不需要移动的分片放回去；将需要pull进来的分片分离；将需要发送出去的数据库条目分离
void ShardKv::updateComeInAndOutShrads(Config config){
    ...
    unordered_set<int> tmpToOutShardMap = this->m_AvailableShards; // 可用的分片
    ...
    for(int i = 0; i < config.shards.size(); i++){
        if(config.shards[i] != this->m_groupId){
            continue;//在新的配置中，该分片不归我这个group管，直接跳过
        }
        if(tmpToOutShardMap.count(i) || oldConfig.configNum == 0){  //原来就归我管，现在还是归我管，或者第一次获取到配置
            tmpToOutShardMap.erase(i);                              //那么保留的就是需要转移给别人的分片数据
            m_AvailableShards.insert(i);                            //那么就不需要pull数据，这类分片直接可用
        }else{
            this->comeInShards[i] = oldConfig.configNum;            //如果是半道上给到自己，那么就需要pull数据，格式为：分片号，old配置编号
        }
    }
    if(this->m_id == 1){
    ...
    if(tmpToOutShardMap.size() > 0){//需要转移出去
        for(const auto& shard : tmpToOutShardMap){//遍历每一个需要转移出去的分片
            unordered_map<string, string> tmpDataBase;//暂存需要发送取出的数据（库）
            tmpDataBase.clear();
            unordered_map<string, string> dataBackUp = this->m_database;

            for(const auto& data : dataBackUp){
                if(key2shard(data.first) == shard){
                    tmpDataBase.insert(data);//暂存需要发送取出的数据（库）
                    this->m_database.erase(data.first);
                }
            }
            this->toOutShards[oldConfig.configNum][shard] = tmpDataBase;//根据不同的分片，归类好要发送取出的数据库
            if(this->m_id == 1){
...
//一个一个分片的进行处理的，在应用层，pull的信息包括：数据库、客户端的请求序列
void ShardKv::updateDataBaseWithMigrateReply(MigrateReply reply){
    ...
    //         m_groupId, reply.shard, comeInShards.size(), reply.database.size(), reply.clientReqId.size());
    // }
    if(!m_AvailableShards.count(reply.shard)){//如果本地可用的分片没有这个
        for(const auto& data : reply.database){
        ...
        garbages[reply.configNum].insert(reply.shard);//标记自己已经成功pull到了这个分片
        // if(this->m_id == 1){
        //     for(auto a : garbages){
        //         int cfgNum = a.first;
        //         for(auto b : a.second){
    ...
    while(1){
        ...
        if(!msg.commandValid){
            ...
            //更新数据,指的是pull回来的数据
                ...
                if(kv->m_requestMap.count(index)){//确保是leader
                    isOpExist = true;
                ...
                if(kv->m_clientSeqMap.count(operation.clientId)){//确保是leader
                    isSeqExist = true;
        ...
        kv->m_raft.postCapSem();
    }
}

//快照的内容有：database、clientSeqMap、comeInShards、toOutShards、m_AvailableShards、garbages、config
...
//获得集群系统内所有group及其内含server的相关信息
vector<vector<kvServerInfo>> getShardKvServerPort(int groupsNum){
    vector<vector<kvServerInfo>> peers(groupsNum, vector<kvServerInfo>(EVERY_SERVER_RAFT));//分为多个组，每个组有五个server
    for(int idx = 0; idx < groupsNum; idx++){
        for(int i = 0; i < EVERY_SERVER_RAFT; i++){
            peers[idx][i].peersInfo.m_peerId = i;//不同group内的server序号是一样递增的
...
int main(){
    ...
    vector<vector<kvServerInfo>> groups = getShardKvServerPort(5);

    ShardKv** kv = new ShardKv*[groups.size()];

    vector<vector<int>> masters = getMastersPort(EVERY_SERVER_RAFT);

    for(int i = 0; i < groups.size(); i++){
        kv[i] = new ShardKv[EVERY_SERVER_RAFT];//单个group内跑着的5个shardkv
        // printf("shardKv%d begin print :\n", i);
        for(int j = 0; j < EVERY_SERVER_RAFT; j++){
            // printf("%d %d %d\n", groups[i][j].peersInfo.m_port.first, groups[i][j].peersInfo.m_port.second, 
            //     groups[i][j].m_kvPort);
                kv[i][j].StartKvServer(groups[i], j, i + 1, 4096, masters);
        }
